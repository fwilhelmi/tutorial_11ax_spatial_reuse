...
% 2.1 - Plot average throughput with average SINR
fig = figure('pos',[450 400 500 350]);
axes;
axis([1 20 30 70]);
plot(1:size(obss_pd_levels,2), tpt_wlan_a, '--', 'LineWidth', 4.0, 'MarkerSize', 12)
...
%   * Plot the tx power in the other yaxis
yyaxis right
plot(1:size(obss_pd_levels,2), sinr_wlan_a, 'x', 'LineWidth', 2.0, 'MarkerSize', 12);%, 'Color', [0.8 0 0]);
hold on
plot(1:size(obss_pd_levels,2), sinr_wlan_b, 'o', 'LineWidth', 2.0, 'MarkerSize', 12);%, 'Color', [0.8 0 0]);
% Tx power
tx_power_a = apply_power_restriction(obss_pd_levels, wlans(1).tx_pwr_ref);
tx_power_b = apply_power_restriction(obss_pd_levels, wlans(2).tx_pwr_ref);
plot(1:size(obss_pd_levels,2), tx_power_a, 'kx', 'LineWidth', 2.0, 'MarkerSize', 12);%, 'Color', [0.8 0 0]);
plot(1:size(obss_pd_levels,2), tx_power_b, 'ko', 'LineWidth', 2.0, 'MarkerSize', 12);%, 'Color', [0.8 0 0]);
hlegend = legend({'WLAN_A (SFCTMN)', 'WLAN_A (Komondor)', 'WLAN_B (SFCTM)', 'WLAN_B (Komondor)', ...
   'Av. SINR (WLAN_A)', 'Av. SINR (WLAN_B)','Tx Power (WLAN_A)', 'Tx Power (WLAN_B)'});
hlegend.NumColumns=4;
ylim([0 max(sinr_wlan_b)])
ylabel('Av. SINR (dB) / Tx Power (dBm)')
ax = gca;
ax.GridAlpha = 0.5;  % Make grid lines less transparent.
% 3.1 - Save Figure
save_figure( fig, 'SIM_1_1b_with_sinr', './Simulations/Output/Toy_scenario_1/' )