...
axis([1, size(obss_pd_levels, 2), 0, 65])
yyaxis left
%   * Plot the average sinr in the other yaxis
yyaxis right
plot(1:size(obss_pd_levels,2), sinr_wlan_a, 'x', 'LineWidth', 2.0, 'MarkerSize', 12);
hold on
plot(1:size(obss_pd_levels,2), sinr_wlan_b, 'o', 'LineWidth', 2.0, 'MarkerSize', 12);
tx_power_a = apply_power_restriction(obss_pd_levels, wlans(1).tx_pwr_ref);
tx_power_b = apply_power_restriction(min(obss_pd_levels)*ones(1, size(obss_pd_levels, 2)), wlans(2).tx_pwr_ref);
plot(1:size(obss_pd_levels,2), tx_power_a, 'kx', 'LineWidth', 2.0, 'MarkerSize', 12);
plot(1:size(obss_pd_levels,2), tx_power_b, 'ko', 'LineWidth', 2.0, 'MarkerSize', 12);
hlegend = legend({'WLAN_A (SFCTMN)', 'WLAN_A (Komondor)', 'WLAN_B (SFCTM)', 'WLAN_B (Komondor)', ...
...
% 3.1 - Save Figure
save_figure( fig, 'SIM_1_1_with_sinr', './Simulations/Output/Toy_scenario_1/' )